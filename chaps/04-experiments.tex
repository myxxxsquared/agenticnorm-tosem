...
Table~\ref{tab:overall-eval} summarizes the overall comparison\footnote{We note that our reproduced results of WebNorm differ slightly from those reported in the original paper. After contacting the authors, we confirmed that they updated their dataset, which improved precision but reduced recall, and the results shown here reflect this corrected version.}.
\lighttechname achieves the highest F1-score on both \trainticket (0.92) and \nicefish (0.95).
Some baselines, such as LogFormer and RAPID, achieve relatively high recall
(e.g., 0.90 on \trainticket and 1.00 on \nicefish for RAPID),
but this comes at the cost of extremely low precision (0.11 / 0.04),
leading to large numbers of false alarms.
LogFormer strikes a better balance, but its F1-scores (0.40 / 0.42) still lag significantly behind ours.
By contrast, WebNorm achieves perfect precision (1.00) but suffers from reduced recall
(0.80 on \trainticket and 0.75 on \nicefish),
causing many real attacks to be missed due to its reliance on fixed rules.
\lighttechname preserves the perfect precision of WebNorm while substantially improving recall
(0.86 / 0.92), thereby delivering the strongest overall detection performance.

\finding{\textbf{RQ1:} \lighttechname outperforms state-of-the-art baselines, achieving the highest F1-scores on both benchmarks.}
...
Prompt refinement shows a similar level of importance, with F1 dropping to 0.61 and 0.83, respectively.
By contrast, removing API relation prediction causes a smaller degradation
(0.92 $\rightarrow$ 0.67 on \trainticket and 0.95 $\rightarrow$ 0.50 on \nicefish),
indicating that it is less dominant but still provides complementary benefits.

\finding{\textbf{RQ2:} Each component of \lighttechname significantly enhances performance, with field clustering and prompt refinement being particularly impactful.}
...
the precision remains consistently perfect (1.00),
...
The invariants produced through our clustering and refinement strategy are robust
enough to transfer across models,
showing that smaller and more efficient LLMs can be deployed in practice
without sacrificing detection accuracy.

\finding{\textbf{RQ3:} \lighttechname maintains high performance across different LLMs, demonstrating its adaptability to smaller, locally deployable models.}

\paragraph{RQ4: Direct Substitution.}
To further validate our design, we directly replace WebNorm’s backbone with smaller LLMs, i.e., \texttt{Gemma 3 4B},
without any of the proposed architectural modifications.
The performance drops sharply in recall. On \trainticket, recall falls from 0.92 to 0.60, and on \nicefish, it plummets from 0.95 to 0.50.
