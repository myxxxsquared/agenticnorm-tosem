...
Table~\ref{tab:tokens-comparison} reports the token counts for each constraints generation task. The reduction is particularly pronounced on \trainticket, as its logs contain more fields, and clustering eliminates a larger portion of redundancy. By shortening the token length, the model can process inputs more efficiently, which in turn leads to higher-quality constraints.
