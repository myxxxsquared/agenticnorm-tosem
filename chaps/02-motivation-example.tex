...
There are two main categories of existing approaches for detecting this type of anomaly:
(1) \textbf{Model-learning-based detectors} that learn normal patterns from historical logs and flag deviations as anomalies; and
(2) \textbf{Invariant-learning-based detectors} that learn semantic invariants from logs and flag violations as anomalies.

Model-learning-based detectors that rely on embeddings struggle here: the abnormal and normal logs are extremely similar, differ in only a few fields, and are embedded very close in vector space.
...
For our case, WebNorm learns the cross-API data constraint:
\[
    \texttt{cancelOrder.arguments.orderId} \in \texttt{queryOrders.results[].id}.
\]
Violation of this invariant flags the attack.
...
\begin{itemize}
    ...
    \item \textbf{Heavyweight LLMs}: constraint confirmation/synthesis relies on closed source property models, adding latency, cost, and compliance concerns, and suffering from privacy issues when sending data to remote servers.
...
\paragraph{Our Method at a Glance}
We aim to retain WebNorm’s \emph{consistency-constraint} power while eliminating its practical blockers.
