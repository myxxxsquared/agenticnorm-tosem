...
\modifypart{
For example, WebNorm can infer the some constraints like \texttt{arguments.orderId} should be a valid UUID.
...
However, due to prompt design and long input logs, WebNorm cannot capture the critical cross-API constraint needed to detect this attack.
}
...
WebNorm suffers from three key limitations:
\begin{itemize}
...
To efficiently adapt logs for lightweight models, we expand each log entity into individual fields and then group comparable ones into clusters.
